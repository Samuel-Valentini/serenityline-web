...
export function VerifyEmailPage() {
  ...
  const submitVerification = useCallback(
    async (tokenToVerify: string) => {
      ...
      try {
        ...
        setStatus("error");
        setError(getVerificationError(verificationError, t("errorFallback")));
      }
    },
    [t],
  );
      ...
      <main className="sl-auth-page">
        ...
        >
          ...
          <div className="d-flex flex-wrap gap-3">
            <Link className="btn btn-primary" to={ROUTES.auth.login}>
              {t("goToLogin")}
            </Link>

            <Link className="btn btn-outline-primary" to={ROUTES.public.home}>
              {t("backHome")}
            </Link>
          </div>
        </section>
    ...
    return (
      <main className="sl-auth-page">
        ...
        >
          ...
          <h1 id="verify-email-loading-title">{t("verifyingTitle")}</h1>
  ...
  return (
    <main className="sl-auth-page">
      <section className="sl-auth-card" aria-labelledby="verify-email-title">
        <p className="sl-eyebrow">SerenityLine</p>
        ...
        <p className="text-muted mb-4">
          {status === "error" ? error?.message ?? t("errorFallback") : t("manualText")}
        </p>
        ...
        <form className="d-grid gap-3" onSubmit={handleManualSubmit}>
          <div>
            <label className="form-label" htmlFor="verificationToken">
              {t("tokenLabel")}
            </label>
            <input
